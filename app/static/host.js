...
async function refresh(probe=false){
  ...
  try{
    const [summary,server]=await Promise.all([
      fetch(`/api/system/summary?probe_simconnect=${probe?'true':'false'}`,{cache:'no-store'}).then(r=>r.json()),
      fetch('/api/server/info',{cache:'no-store'}).then(r=>r.json())
    ]);
...
function _setFieldConcealed(input, concealed){
  ...
  if(concealed && !button){
    ...
    button.addEventListener('click',()=>{
      ...
      if(document.getElementById('hostStreamerMode')?.checked && !confirm('Streamer Mode is enabled. This may reveal sensitive information on stream or in screenshots. Reveal anyway?'))return;
...
async function refreshSurfaceStatus(force=false){
  ...
  if(!box)return;
  try{
    const response=await fetch(force?'/api/livemap/surface/rescan':'/api/livemap/status',{method:force?'POST':'GET',cache:'no-store'});
...
async function loadSettings(){const response=await fetch('/api/settings',{cache:'no-store'});if(!response.ok)throw new Error(`Settings HTTP ${response.status}`);fillSettings(await response.json())}
...
async function revokeAllDevices(){if(!confirm('Revoke every paired LAN device?'))return;await fetch('/api/security/devices',{method:'DELETE'});await refreshSecurity()}
async function refreshVpilotInstall(){
  try{
    const response=await fetch('/api/vpilot/install/status',{cache:'no-store'});const data=await response.json();if(!response.ok)throw new Error(data.detail||`HTTP ${response.status}`);
