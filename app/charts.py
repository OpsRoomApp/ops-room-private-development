...
def _chartfox_reset_metrics() -> None:
    """v0.25.55: reset all runtime metrics to zero. Called after successful
...
def chartfox_debug(chart_id: str = "", airport: str = "") -> dict:
    """v0.25.55: enhanced debug endpoint with live API response inspection.
...
def _chartfox_overlay_compute(georef, display_width_px, display_height_px):
    ...
    return {"ok": True, "x_px": round(px, 1), "y_px": round(py, 1),
            ...
            "scope_warning": "v0.25.55: charts:geos scope not available (ChartFox returns 403); georefs still parsed from chart detail responses"}
...
def ownship_overlay_status():
    ...
    return {"ok": bool(tel.get("ok")),
            ...
            "message": "Live position available; chart overlay requires georef params per chart. v0.25.55: charts:geos scope not available from ChartFox."}
...
# v0.25.55: file-cache read/write counters for debug endpoint.
...
def _chartfox_cache_get(chart_id: str) -> tuple[bytes | None, str | None, str | None]:
    ...
    v0.25.55 (round 2): iterates ALL cached chunks and sniffs bytes (with a
...
def chartfox_force_cache_cleanup() -> dict:
    """v0.25.55: delete ALL cached ChartFox temp files (chart PDFs, images, stale
    ...
    def _download_and_cache(url: str, is_pdf: bool) -> dict | None:
        ...
        try:
            ...
            _LOGGER.warning("[CHARTFOX PROXY] download_failed cid=%s url=%s error=%s",
                            cid, url[:80], exc)
            return None
        # v0.25.55 (round 2): the previous additive gate (reject if claimed
