...
cd /d "%~dp0"
title Build OPS ROOM 0.25.51 Public Release Complete Package

echo ================================================================
echo OPS ROOM 0.25.51 Public Release build
...
"%VENV_PY%" tools\write_update_manifest.py --version 0.25.51 --channel stable --zip "%DIST_DIR%\OPS_ROOM_v0_25_48_Public_Windows_x64.zip" --out "%DIST_DIR%\update.json" || goto :fail
...
:fail
echo.
echo OPS ROOM 0.25.51 Public Release complete build failed. Review the first ERROR above.
pause
exit /b 1
