# OPS ROOM v0.25.55 — POST-JULY 30 RECONSTRUCTION AUDIT

## Audit Date: 2026-08-01
## Backup Baseline: July 30, 2026 (v0.25.35)
## Target: v0.25.55 final state

---

## Critical Caveat: Version Comments Are Misleading

During recovery, `sed` replaced ALL occurrences of `0.25.35` with `0.25.55` across the codebase — including in code comments. Comments like `// v0.25.55: resize annotation canvas` were originally `// v0.25.35: resize...` and do NOT indicate post-July 30 work. **Only code structure and features — not version strings in comments — are reliable evidence.**
...
| v0.25.55 | Aug 1 | Built-in seed coordinates, zone sweep fallback, per-record normalization isolation, populated pipeline stats, search index fallback, discovery strategy tracking, structured logging, regression tests |
...
| Aspect | Expected (v0.25.55) | Actual (Restored) | Status |
|--------|---------------------|-------------------|--------|
...
| Feature | Expected (v0.25.55) | Actual (Restored) | Status |
|---------|---------------------|-------------------|--------|
...
| PDF + annotation pan together | `v0.25.55 comment about panWrap` | ✅ Present (line 2161) | MATCH |
...
|--------|----------|--------|--------|
| `version.json` | 0.25.55 | ✅ 0.25.55 | MATCH |
| `main.py` FastAPI version | 0.25.55 | ✅ 0.25.55 | MATCH |
| `updater.py` DEFAULT_VERSION | 0.25.55 | ✅ 0.25.55 | MATCH |
| Build scripts | 0.25.55 | ✅ 0.25.55 | MATCH |
| UI labels (index.html, host.html) | 0.25.55 | ✅ 0.25.55 | MATCH |
| Cache-busters (`v=0-25-47`) | 0.25.55 | ✅ Correct counts | MATCH |
...
| `BUILD *.bat` | 0.25.55 outputs | ✅ Correct | MATCH |
...
| 7 | FastAPI 0.25.55, 239 routes | `main.py` |
...
| 11 | All build infrastructure at 0.25.55 | `*.bat`, `*.spec`, `*.iss` |
