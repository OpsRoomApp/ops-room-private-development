...
# ---------------------------------------------------------------------------
# Release/build metadata - bumped to v0.24.108 Black Box RC6
# ---------------------------------------------------------------------------
...
check("version metadata is v0.24.108 Black Box RC6", version == {
    "product": "OPS ROOM",
    "version": "0.24.108",
    "build": "public-beta-black-box-release-candidate-6",
    "codename": "Flight Recorder Telemetry Refresh",
    "channel": "release-candidate",
})
check("runtime and launcher target v0.24.108", 'FastAPI(title="OPS ROOM", version="0.24.108")' in main_text and "Starting OPS ROOM v0.24.108" in launcher_text)
check("updater manifest targets RC6 package", manifest.get("version") == "0.24.108" and str(manifest.get("download_url", "")).endswith("OPS_ROOM_v0_24_108_Public_Beta_Black_Box_RC6_Windows_x64.zip") and manifest.get("sha256") == "TO_BE_FILLED_BY_BUILD_SCRIPT")
check("short temp roots are intermediates only (OR108)", "%TEMP%\\OR108" in complete and "%TEMP%\\OR108" in windows and "%TEMP%\\OR108" in camera)
check("build scripts agree on RC6 package", complete.count("OPS_ROOM_v0_24_108_Public_Beta_Black_Box_RC6_Windows_x64.zip") >= 3 and windows.count("OPS_ROOM_v0_24_108_Public_Beta_Black_Box_RC6_Windows_x64.zip") >= 3 and "Flight Recorder Telemetry Refresh" in text("tools/write_update_manifest.py"))
...
check("adapter schema version stays at 0.24.106 (catalogue unchanged, BC-preserving)", text("app/aircraft_adapter_installer.py").count('ADAPTER_VERSION = "0.24.106"') == 1 and text("app/aircraft_adapter_installer.py").count('ADAPTER_VERSION = "0.24.108"') == 0)

# ---------------------------------------------------------------------------
# RC6-specific behavioural checks (the seven-item user request)
# ---------------------------------------------------------------------------

# 1. FSUIPC log auto-silencing at startup.
...
check(
    ...
    (cleanup_tests.stdout + cleanup_tests.stderr).strip() if cleanup_tests.returncode else "7 focused tests",
)

# 2. Engineering telemetry Controls tab.
check("Controls tab renders sidestick/yoke crosshair", "bb-widget-crosshair" in ui and "crosshairX" in ui and "crosshairY" in ui)
check("Controls tab renders twin throttle levers", "bb-widget-throttles" in ui and "bb-throttle-levers" in ui and "LVR" in ui)
check("Controls tab renders rudder pedals", "bb-widget-rudder" in ui and "RUDDER PEDALS" in ui)
...
# 3. SkyDolly design credits retained.
...
# 4. DATA GAP event flooding removed.
bb_text = text("app/black_box.py")
check("DATA GAP event emission removed from black_box.py", '_event_locked(active, "DATA GAP"' not in bb_text and 'event(active, "DATA GAP"' not in bb_text and "_event_locked(_ACTIVE, \"DATA GAP\"" not in bb_text and "DATA GAP events were removed at user request" in bb_text)

# 5. Black Box UI de-technicalised.
...
check("TECHNICAL DETAILS summary replaced with friendlier 'More details'", "TECHNICAL DETAILS" not in ui and "More details" in ui and "Saved file" in ui and "Recording tag" in ui and "What was captured" in ui or "What was captured" in ui)
...
# 7. Version bump to v0.24.108 RC6 (covered by metadata check above).
check("UI version label is v0.24.108 Black Box RC6", "OPS ROOM 0.24.108 BLACK BOX RC6" in html and "v=0-24-108-blackbox-rc6" in html)

# ---------------------------------------------------------------------------
# Carryover from RC5: catalogue, enrichment, perf-hotspot
# ---------------------------------------------------------------------------
...
# v0.24.107 perf-hotspot patch (registry caching, static PMDG meta) still present.
...
# Central telemetry enrichment preservation.
...
# PMDG official SDK merge (hoisted static meta table) still correct.
...
# Protected operational/replay modules must be byte-identical to the RC5
# baseline (verified at RC5 release), to ensure the RC6 telemetry refresh
# did not regress any operational GSX/Fenix/announcement/replay engine.
# gsx_remote.py was patched under RC5; that hash remains pinned.
protected = {
    ...
    "app/addon_telemetry.py": "0eccb9d22ed0aed2d3e019892633a9be53f72cc841ec48b89384f01584682161",
    "app/pmdg777_sdk.py": "82e3100b5c613a3ce5e7e6116ce574b5d86fbd94d01db441f6eedf019fa8b6c9",
    "app/aircraft_adapter_catalog.py": "5080fa8421cfe6c15fc407f4f1dd2c958574e115901154655529454a1f0c502d",
    "app/pmdg777_eula.py": "5ae60432f34e5126a6777b2a43231885d13b781b3513e88e70a74c1d347358e5",
}
ok, detail = True, ""
for path, digest in protected.items():
    ...
    if actual != digest:
        ...
        break
check("protected operational/replay modules are byte-identical to the RC5 baseline (RC6 refreshed only main.py/black_box.py + static assets)", ok, detail)
