...
# Website-primary downloads with a GitHub release copy as the fallback.
...
check(
    ...
    and manifest.get("download_url") == WEBSITE_DOWNLOAD_URL
    and manifest.get("url") == WEBSITE_DOWNLOAD_URL
    and manifest.get("fallback_download_url") == FALLBACK_DOWNLOAD_URL
...
if ARGS.dist:
    ...
    check(
        "generated manifest has the real ZIP SHA-256, website-primary URLs and a GitHub fallback",
        bool(actual_sha)
        ...
        and generated_manifest.get("download_url") == WEBSITE_DOWNLOAD_URL
        and generated_manifest.get("url") == WEBSITE_DOWNLOAD_URL
        and generated_manifest.get("fallback_download_url") == FALLBACK_DOWNLOAD_URL
        and generated_manifest.get("sha256") == actual_sha
        and generated_manifest.get("sha256") != "TO_BE_FILLED_BY_BUILD_SCRIPT"
        and (not generated_manifest.get("installer_url") or generated_manifest.get("installer_url") == WEBSITE_INSTALLER_URL)
        and (not generated_manifest.get("installer_url") or generated_manifest.get("fallback_installer_url") == GITHUB_INSTALLER_URL),
        f"sha256={actual_sha}",
    )
    check(
