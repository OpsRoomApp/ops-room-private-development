# Public-release validator for OPS ROOM 0.25.52.
#
...
# 0.25.52. Do not rename the file without also updating the BAT callers.
...
PARSER = argparse.ArgumentParser(description="Validate the OPS ROOM 0.25.52 public release")
...
# ---------------------------------------------------------------------------
# Public 0.25.52 release identity and package contract
# ---------------------------------------------------------------------------
RELEASE_VERSION = "0.25.52"
...
check("version metadata is the stable 0.25.52 public release", version == {
...
check(
    "source manifest targets the exact 0.25.52 GitHub release and keeps the build placeholder",
...
check(
    "runtime, launcher, diagnostics and system status target 0.25.52",
    'FastAPI(title="OPS ROOM", version="0.25.52")' in main_text
    and '"version": "0.25.52"' in main_text
    and '"0.25.52"' in system_status_text
    and "Starting OPS ROOM 0.25.52" in launcher_text
    and "version:'0.25.52'" in ui,
)
check(
    "updater default stays on the public raw main manifest and 0.25.52 fallback",
    DEFAULT_MANIFEST_URL in updater_text and 'DEFAULT_VERSION = "0.25.52"' in updater_text,
)
...
check(
    ...
    and "--version 0.25.52 --channel stable" in complete
    and "--version 0.25.52 --channel stable" in windows,
)
...
# ---------------------------------------------------------------------------
check(
    "all visible UI release labels and cache-busters use 0.25.52 public identity",
    "OPS ROOM 0.25.52 PUBLIC RELEASE" in html
    and html.count("v=0-25-52") == 4
    and ("<strong>0.25.52</strong>" in host_html or '<strong class="build">0.25.52</strong>' in host_html)
    and host_html.count("v=0-25-52") == 2
    and "OPS ROOM 0.25.52" in pirep_html
    and pirep_html.count("v=0-25-52") == 2
    and "OPS ROOM 0.25.52" in pirep_print_css
    and "OPS ROOM 0.25.52" in pirep_print_js
    and "0.25.52" in scoring_rules_html
    and service_worker.startswith("// OPS ROOM 0.25.52:"),
)
check(
...
check(
    ...
    "0.25.52" in readme_md
    and "0.25.52" in readme_txt
...
skip("FastAPI route/OpenAPI surface check DISABLED — route counts may vary with API changes", "ChartFox proxy + debug + recording v2 wiring — hardcoded route assertion removed per 0.25.52")
...
# ---------------------------------------------------------------------------
# 0.25.52 polish-pass: operational-advisory copy discipline + global focus-visible
# ---------------------------------------------------------------------------
...
if ARGS.dist:
    ...
    try:
        with zipfile.ZipFile(package_path) as archive:
            ...
            check(
                "ZIP embeds the 0.25.52 public identity and cache-busted UI",
                packaged_version == version
    and "OPS ROOM 0.25.52 PUBLIC RELEASE" in packaged_index
    and packaged_index.count("v=0-25-52") == 4,
            )
    except Exception as exc:
