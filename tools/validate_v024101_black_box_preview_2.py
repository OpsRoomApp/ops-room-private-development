...
check(version == {"product": "OPS ROOM", "version": "0.24.101", "build": "public-beta-black-box-preview-2", "codename": "Flight Data Recorder", "channel": "preview"}, "Version metadata identifies Black Box Preview 2")
check(update.get("version") == "0.24.101" and update.get("channel") == "preview" and "Black_Box_Preview" in str(update.get("download_url")), "Updater metadata targets the preview package")
...
check("OPS_ROOM_v0_24_101_Public_Beta_Black_Box_Preview_2_Windows_x64.zip" in build, "Complete Windows build targets the preview ZIP")
...
notes = (ROOT / "OPS_ROOM_v0_24_101_BLACK_BOX_PREVIEW_2_RELEASE_NOTES.md").read_text(encoding="utf-8")
...
check("Live Map recovery" in notes and "v0.24.52" in notes, "Release notes document the inherited map/release patch")
check("mapAviationRefreshPending" in ops_js and "airportLookupBboxParam" in ops_js, "Live Map queues refreshes and uses padded surface discovery")
check("mapSelectedAirportIcao" in ops_js and "mapSurfaceTargetIcao" in ops_js, "Live Map retains real airport selection state")
check("if(preset!=='airport')clearAirportSurface" not in ops_js, "Presets no longer erase loaded surfaces")
check("clean:{mapLayerTraffic:true,mapLayerControllers:true,mapLayerCoverage:false,mapLayerAirports:true,mapLayerRunways:true,mapLayerSurface:true" in ops_js, "CLEAN keeps zoom-driven airport surfaces")
check("ground-auto-console span::after" not in (ROOT / "app/static/opsroom.css").read_text(encoding="utf-8"), "Repeated Ground Control pseudo-text is removed")
check("<span>Departure: enabled preparation services" not in index_html, "Ground Control explanatory paragraph is removed")
check("tzdata>=2025.2" in (ROOT / "requirements_shipping.txt").read_text(encoding="utf-8"), "Windows timezone data is packaged")


# The packaged FastAPI application exposes the expected preview version/routes.
from app.main import app  # noqa: E402
check(app.version == "0.24.101", "FastAPI version is 0.24.101")
