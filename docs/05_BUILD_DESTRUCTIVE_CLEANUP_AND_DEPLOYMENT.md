# OPS ROOM v0.25.58 — Build, Packaging, Cleanup & CI/CD
...
```json
{
  ...
  "fallback_download_url": "https://github.com/OpsRoomApp/ops-room-releases/releases/download/v0.25.58/OPS_ROOM_v0_25_00_Public_Windows_x64.zip",
  "sha256": "AFA80454495CEE29F4CB92856F73EBB2C633DB46A4EA493F467365F0987CAC3D",
  "release_notes": "OPS ROOM v0.25.58 is available."
}
```
...
|---|---|---|
| **Version Consistency** | ~12 | All Python/JS/HTML/CSS/batch files reference `0.25.58`, no stale version strings |
...
| **Route Surface** | ~1 | **SKIP** — hardcoded route count assertion removed per 0.25.58 policy |
...
```
PASS: version metadata is the stable v0.25.58 public release
PASS: source manifest targets the exact v0.25.58 GitHub release
PASS: runtime, launcher, diagnostics and system status target v0.25.58
...
...
|---|---|---|
| `app/main.py` | FastAPI version, diagnostics version, updater fallback | `"0.25.58"` |
| `app/charts.py` | ChartFox diagnostics version | `"0.25.58"` |
| `app/realworld.py` | Search engine version comment | `"0.25.58"` |
| `app/system_status.py` | System summary version | `"0.25.58"` |
| `app/updater.py` | `DEFAULT_VERSION` | `"0.25.58"` |
| `app/static/opsroom.js` | In-app version stamp | `"0.25.58"` |
| `app/static/opsroom.css` | Cache-buster comment | `/* v0.25.58 */` |
| `app/static/index.html` | Script/link cache-buster | `?v=0-25-00` |
| `app/static/service-worker.js` | SW version | `"0.25.58"` |
| `version.json` | Build manifest version | `"0.25.58"` |
| `update.json` | Update manifest version | `"0.25.58"` |
| `opsroom_launcher.py` | Launcher version | `"0.25.58"` |
| `README.md` | Public readme version | `v0.25.58` |
...
| `tools/validate_*.py` | Validation target version | `"0.25.58"` |
| `tools/write_update_manifest.py` | Manifest version | `"0.25.58"` |
...
# Version consistency
grep -r "0.25.58" --include="*.py" --include="*.js" --include="*.bat" --include="*.json" --include="*.html" --include="*.css"
